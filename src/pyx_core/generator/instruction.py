...
2. shell-usage: Teaches LLM how to use the current shell correctly
"""
...
def generate_pyx_instructions(
    ...
    Args:
        info: EnvironmentInfo object. If None, will be collected.
        show_progress: Show progress bars when collecting info
        style: 'file' (recommended) or 'base64' (legacy)
    ...
    lines = _build_lines_base64(info, env_keys_with_usage) if style_normalized == "base64" else _build_lines_file(info, env_keys_with_usage)
...
def _build_lines_base64(info: EnvironmentInfo, env_keys_with_usage: dict[str, str]) -> list[str]:
    """Build markdown lines for base64 style (legacy)."""
    lines: list[str] = []
    ...
    if info.global_env_path:
        ...
        from ..environment import _USER_ENV_PATH
...
def _build_lines_file(info: EnvironmentInfo, env_keys_with_usage: dict[str, str]) -> list[str]:
    """Build markdown lines for file-first style (recommended)."""
    lines: list[str] = []
    ...
    lines.append("3. All outputs MUST be written to files (prefer `.txt` for human-readable summaries).")
    ...
    lines.append("")
    lines.append("Example variants: `fetch_rates.a.input.json` -> `fetch_rates.<run_id>.manifest.json` + `fetch_rates.<run_id>.log.txt` + dynamic outputs (tracked by the manifest).")
    lines.append("")
    ...
    # Keep environment variables + command list sections (still useful)
    lines.append("---")
    lines.append("")
    lines.append("## Environment Variables (Auto-loaded by pyx)")
    lines.append("")
