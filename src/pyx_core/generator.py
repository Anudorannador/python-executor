...
def generate_pyx_instructions(
    ...
    def _build_lines_file() -> list[str]:
        ...
        lines.append("4. Stdout MUST be a short summary only: output path + file size + tiny preview.")
        ...
        lines.append("## Output Explosion Control (Input JSON + Output File)")
        lines.append("")
        ...
        lines.append("- **Output**: always write to an output file (usually `.txt`; sometimes `.json`)")
        ...
        lines.append("- Output: `.temp/<task>.<variant>.output.txt`")
        lines.append("")
        lines.append("Example variants: `fetch_rates.a.input.json` -> `fetch_rates.a.output.txt`, `fetch_rates.a2.input.json` -> `fetch_rates.a2.output.txt`.")
        lines.append("")

        lines.append("### CLI Support (Recommended)")
        lines.append("")
        lines.append("`pyx run` supports `--input-path` and `--output-path` to standardize I/O.")
        lines.append("It exposes these paths to your code via environment variables:")
        lines.append("")
        lines.append("- `PYX_INPUT_PATH` (optional)")
        lines.append("- `PYX_OUTPUT_PATH` (always set by the CLI)")
        lines.append("")
        lines.append("By default, if you do not pass `--output-path`, the CLI writes to `.temp/<task>.<timestamp>.output.txt`.")
        lines.append("The CLI prints only a short summary to stdout to avoid token blow-ups.")
        lines.append("")
